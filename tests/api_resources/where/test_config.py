...
class TestConfig:
    ...
    def test_method_retrieve(self, client: OpenTransit) -> None:
        config = client.where.config.retrieve()
...
class TestAsyncConfig:
    ...
    async def test_method_retrieve(self, async_client: AsyncOpenTransit) -> None:
        config = await async_client.where.config.retrieve()
        assert_matches_type(ConfigRetrieveResponse, config, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncOpenTransit) -> None:
        response = await async_client.where.config.with_raw_response.retrieve()
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncOpenTransit) -> None:
        async with async_client.where.config.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
